	        	...
	        	<section id="p10">
	        		<div  class="page-header">
						...
						<p>
							...
							연동할 수 있는 다중 채널을 제공하며, 이를 통해 개발자는 UI 기술과는 독립적으로 동일한 인터페이스를 통해 서버 측 애플리케이션을 개발할 수 있다.<br/>
							현재 제공하는 UI는 <b>웹표준, JSON, Miplatform, XPlatform, WebPlus, WebSquare, Mybuilder Flex, EXT-JS, Gauce</b> 등이 있으며
							본 예제에서는 JSON과 표준웹 방식을 예제로 설명한다.<br/><br/>
							관련 코드는 com.jcf.sample.sua 패키지에 자세히 나와있다.
						</p>
						<pre class="prettyprint linenums">

&lt;!-- jcf-sua 를 사용한 간편한 xml 설정  --&gt;
&lt;!-- channel-type 등록을 통해 다양한 UI를 지원한다. --&gt;
&lt;jcfsua:sua-config auto-config="false" webflow-prefix="/WEB-INF/views/" webflow-suffix=".jsp"&gt;
	...
	&lt;jcfsua:custom-interceptor ref="추가 interceptor..."/&gt;
&lt;/jcfsua:sua-config&gt;
						</pre>
					</div>
				</section>
				<section id="p11">
					<div  class="page-header">
						<h3>MCI Request</h3>
						<p>
							JCF-SUA 서비스를 통해 웹 요청을 받는 방식에 대해서 설명한다.<br/>
							JCF 는 Request 요청을 <b>MciRequest</b> 인터페이스를 통해 받을 수 있으며, 아래의 다양한 메서드 및 실습 예제을 통해 학습할 수 있다.
						</p>
						<pre class="prettyprint linenums">

public interface MciRequest {
	...
	// Dataset Id에 해당하는 Data를 모델 형태로 변환하여 반환한다.
	&lt;E&gt; E get(String datasetId, Class&lt;E&gt; clazz);

	// 요청 받은 파라미터 MAP을 모델 클래스로 변환하여 반환한다.
	&lt;T&gt; T getParam(Class&lt;T&gt; type);

	// Dataset 아이디에 해당하는 데이터를 Map 형태로 변환하여 반환한다.
	Map&lt;String, ?&gt; getMap(String datasetId);

	/**
	 * 요청한 Dataset이 행의 상태정보(rowStatus)를 포함하고 있다면
	...
	response.addSuccessMessage("요청받은 Map 유형의 데이터는 아이디 : " + name + " 나이 : " + age + " 등록일자 : " + regiDate + " 입니다.");
	          		...
	          		<section id="p11_6">
						...
						<div class="bs-docs-example">
							<br />
							<div  id="p11_6_iframe">
	                        	<iframe src="/jcf-showcase/openpage.do?viewname=p1_iframe_2"  width="500" height="150" name="main" style="border:0;"></iframe>
							</div>
							<br />
						</div>
						<pre class="prettyprint linenums">
	...
	response.addSuccessMessage("요청 받은 URL 주소는 " + requestUrl + "입니다.");
		       	...
		       	<section id="p12">
					<div class="page-header">
						<h3>MciResponse</h3>
						<p>
							JCF-SUA 서비스를 통해 웹에 데이터를 보내는 방식에 대해서 설명한다.<br>
							JCF 는 Response 응답을 <b>MciResponse</b> 인터페이스를 통해 보낼 수 있으며, 아래의 다양한 메서드 및 실습 예제을 통해 학습할 수 있다.
						</p>
						<pre class="prettyprint linenums">
...
public interface MciResponse {
	...
	// 서버에서 조회한 모델(bean)정보를 가지고 DatasetId로 Dataset을 생성하여 화면에 전송한다.
	&lt;E&gt; void set(String datasetId, E bean);

	// 서버에서 조회한 Map 정보를 가지고 DatasetId로 Dataset을 생성하여 화면에 전송한다.
	void setMap(String datasetId, Map&lt;String, ?&gt; map);

	// 서버에서 조회한 모델을 담고 있는 List 정보를 주어진 DatasetId로 Dataset을 생성하여 화면에 전송한다.
	&lt;E&gt; void setList(String datasetId, List&lt;E&gt; listOfModel);

	// 서버에서 조회한 데이터을 담고 있는 Map List 정보를 주어진 DatasetId로 Dataset을 생성하여 화면에 전송한다.
	void setMapList(String datasetId, List&lt;? extends Map&lt;String, ?&gt;&gt; mapList);

	// JSP View 에서, 출력될 페이지를 ViewName을 통하여 지정할 수 있다.
	...
	response.setMapList("DS_OUT", list);
	      			...
	      			<section id="p12_6">
						...
						<div class="bs-docs-example" id="p12_6">
					   		<iframe src="openpage.do?viewname=p1_iframe_3" width="500" height="150" name="main" name="main" style="border:0;"></iframe>
						</div>
						<pre class="prettyprint linenums">
