 ...
 * @author mina
 * @since 2009/11/17
 * @Description  PropertyService class

 *  Copyright (C) 2009 by disc  All right reserved.
 */
...
//		getConfiguration().values();
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (key == null || key.equals("")) {
					if (PropertyService.LOGGER.isErrorEnabled())
						PropertyService.LOGGER.error(messageSource
	...
	private void loadPropertyRes(Resource resource, String encoding)
		...
		if (PropertyService.LOGGER.isDebugEnabled()) {
			PropertyService.LOGGER.debug(messageSource.getMessage(
					"debug.properties.filename", new String[] {
							resource.getFilename(), encoding }, Locale
							.getDefault()));
		}
